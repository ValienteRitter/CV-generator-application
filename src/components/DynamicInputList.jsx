    ...
    return (
        <div className="dynamic-input-list">
            <button className="add-input" onClick={(e) => {
                ...
                {data.map(({id, value}) => <DynamicInput key={id} type={type} handleRemoveItem={() => handleRemoveItem(type, isEducation ? 'skills' : 'responsibilities', id)} handleUpdateItem={(e) => handleUpdateItem(e, type, isEducation ? 'skills' : 'responsibilities', id)}/>)}
        </div>
    )
}

export function DynamicInput({type, handleRemoveItem, handleUpdateItem}) {
    const isEducation = type === 'education' 
    return (
